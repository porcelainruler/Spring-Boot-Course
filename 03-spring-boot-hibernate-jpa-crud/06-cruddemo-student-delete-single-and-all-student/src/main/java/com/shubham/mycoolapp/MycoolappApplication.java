...
public class MycoolappApplication {
	...
	public CommandLineRunner commandLineRunner (StudentDAO studentDao) {
		return runner -> {
			...
			// getAllStudentsWithGivenLastName(studentDao);
	...
	private void deleteAllStudent(StudentDAO studentDao) {
		...
		int delCount = studentDao.deleteAll();

		// Log the count of student deleted
		System.out.println("Deleted student count: " + delCount);
	}
	...
	// Fetch the student from db, make change and save it to db
	private void updateStudent(StudentDAO studentDao) {
		...
		student.setFirstName("Adarsh2");

		// Save it in DB
		System.out.println("Saving / Persisting the student in DB...");
	...
	// Fetch list of student with particular latName
	private void getAllStudentsWithGivenLastName(StudentDAO studentDao) {
		// Fetch students list with given last name
		List<Student> fetchedStudents = studentDao.findByLastName("Kumar");

		// Display list
		for (Student tempStudent : fetchedStudents) {
